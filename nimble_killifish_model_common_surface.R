...
myFunctionLPLP <- nimbleFunction(
  run = function(H = double(0), LP = double(0), philength = double(0), rholength = double(0), density.LP = double(0), le.comp.LP = double(1), le.focal.LPLP = double(1)) {
    numNumber <- LP * (sum( ((exp(philength * le.comp.LP - philength * le.focal.LPLP) ) *
                               exp((-(rholength * le.comp.LP - rholength * le.focal.LPLP)^2) / (2*H^2) ))[1:density.LP]
    ) ) / 1.4
...
myFunctionKOKO <- nimbleFunction(
  run = function(H = double(0), KO = double(0), philength = double(0), rholength = double(0), density.KO = double(0), le.comp.KO = double(1), le.focal.KOKO = double(1)) {
    numNumber <- KO * (sum( ((exp(philength * le.comp.KO - philength * le.focal.KOKO) ) *
                               exp((-(rholength * le.comp.KO - rholength * le.focal.KOKO)^2) / (2*H^2) ))[1:density.KO]
    ) ) / 1.4
...
code1 <- nimbleCode({
  ...
  #=====================================================================================
  philength ~ dnorm(0.03, 0.010) #From Analysis with Interaction
  rholength ~ T(dnorm(0.03, 0.005), 0,1) #From Analysis with Interaction
  H <- 1 #~ dnorm(1, 100)
  ...
  for(i in 1:n_obs) {
    intsurf.LPLP[i] <-  myFunctionLPLP(1, LP[i], philength, rholength, density.LP[i], le.comp.LP[, chan.num[i]],le.focal.LPLP[i,])
    
    intsurf.KOKO[i] <- myFunctionKOKO(1, KO[i], philength, rholength, density.KO[i], le.comp.KO[, chan.num[i]],le.focal.KOKO[i,])
...
inits <- list(
  ...
  f.beta = rep(0, 6),
  d.beta = rep(0, 6),
  philength = 0.03,#rnorm(1, 0, 0.25),
  rholength = 0.03,#rnorm(1, 0, 0.25),
  H=1,
  g.sigma = 1,
  d.sigma = 1,
